class Solution {
    public int coinChangeHelper(int index, int amount, int[]coins, int memo[][]){
        ...
        if(index == coins.length || amount < 0) return 1000000000;

        if(memo[index][amount] != -1) {
            return memo[index][amount];
        }
        
        int take = 1+coinChangeHelper(index, amount-coins[index], coins, memo);
    ...
    public int coinChange(int[] coins, int amount) {
        ...
        int ans = coinChangeHelper(0, amount, coins, memo);
